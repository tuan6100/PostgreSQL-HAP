...
CREATE INDEX idx_seat_class_aircraft ON "Seat" ("seat_class_id", "airline_id");
...
BEGIN
        WHILE current_date <= end_date LOOP
                ...
                partition_name := 'Flight_' || TO_CHAR(current_date, 'YYYYMM');
                sql_command := 'CREATE TABLE IF NOT EXISTS "' || partition_name || '" PARTITION OF "Flight" ' ||
                                'FOR VALUES FROM (''' || current_date || ' 00:00:00'') TO (''' || next_date || ' 00:00:00'');';
                EXECUTE sql_command;
                current_date := next_date;
        END LOOP;
END;
$$;
CALL create_monthly_flight_partitions(CURRENT_DATE, CURRENT_DATE + INTERVAL '12 months');
...
CREATE INDEX idx_customer_info ON "Customer" (full_name, phone_number);
